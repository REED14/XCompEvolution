...
typedef struct __attribute__((packed)) BFHeader{
  __uint16_t HF;
  __uint32_t BMPSize;
...
typedef struct __attribute__((packed)) BIHeader{
  __uint32_t hsize;
  __uint32_t bmp_width;
  __uint32_t bmp_height;
  __uint16_t ncp;
  __uint16_t depth;
...
int use_bkg_pic = 0;
int wdata, hdata, ddata;
XImage *backOfTheWindow;
Picture underBKG;
...
XImage *BMP_READER(char *filename, Display *dpy, int *height, int *width, int *depth, unsigned char *bitmap_free){
  //variable declaration
  FILE *reader;
  BFHeader bitmapFHeader;
  BIHeader bitmapIHeader;

  //reader avalability check
  reader = fopen(filename, "rb");
  if(reader == NULL)
    return NULL;

  //read bitmap file header
  fread(&bitmapFHeader, sizeof(BFHeader), 1, reader);
  fread(&bitmapIHeader, sizeof(BIHeader), 1, reader);

  //going to where the bitmap begins
  int seek_value = bitmapFHeader.Offset;
  int bmpar_size = bitmapFHeader.BMPSize - bitmapFHeader.Offset;
  fseek(reader, seek_value, SEEK_SET);

  *height = bitmapIHeader.bmp_height;
  *width = bitmapIHeader.bmp_width;
  *depth = bitmapIHeader.depth;

  //bitmap array structure
  unsigned char *bitmap_img;
  unsigned char temp_img;
  bitmap_img = (unsigned char*)malloc(bitmapIHeader.bmp_width * bitmapIHeader.bmp_height * sizeof(unsigned char)* 3);
  fread(bitmap_img, sizeof(unsigned char)*bmpar_size, 1, reader);
  
  //bitmap array reconstruction
  
  for(int i = 0; i < bmpar_size; i+=3){
    temp_img = bitmap_img[i];
    bitmap_img[i] = bitmap_img[i+2];
    bitmap_img[i+2] = temp_img;
  }
  
  fclose(reader);

  unsigned char *bitconv = (unsigned char *)malloc(bitmapIHeader.bmp_width * bitmapIHeader.bmp_height * sizeof(unsigned char)* 4);
  int something = 0;
  
  for(int i=bmpar_size-1; i>=0; i-=3)
    {
      bitconv[something] =  bitmap_img[i];
      bitconv[something+1] = bitmap_img[i+2];
      bitconv[something+2] = bitmap_img[i+1];
      bitconv[something+3] = 255;
      something += 4;
    }

  unsigned char tempR, tempG, tempB;
  unsigned int antix=0;
  int xus=0, yus=0;
  for(int i=0; i<bmpar_size/3*4; i+=4){
      xus = (i/4)%bitmapIHeader.bmp_width;
      yus = (i/4)/bitmapIHeader.bmp_width;
      antix = (yus*bitmapIHeader.bmp_width)*4+bitmapIHeader.bmp_width*4-xus*4;
     
      tempB = bitconv[antix];
      bitconv[antix] = bitconv[i];
      bitconv[i]=tempB;

      tempG = bitconv[antix+1];
      bitconv[antix+1] = bitconv[i+1];
      bitconv[i+1]=tempG;

      tempR = bitconv[antix+2];
      bitconv[antix+2] = bitconv[i+2];
      bitconv[i+2]=tempR;

      if(bitmapIHeader.bmp_width-xus<=xus)
	i=(yus+1)*bitmapIHeader.bmp_width*4;
  }
  
  XImage *img_thing = XCreateImage(dpy, XRenderFindStandardFormat(dpy, PictStandardARGB32),
				   32, ZPixmap, 0, bitconv,
				   bitmapIHeader.bmp_width, bitmapIHeader.bmp_height,
				   32, 0);

  bitmap_free = bitmap_img;
  if(!img_thing)
    {
      return NULL;
    }
...
static Picture Bitmap_Transformer(Display *dpy, XImage *bmpImage, unsigned int height, unsigned int width, unsigned int depth, unsigned char* bitmap_free){
  Pixmap  bmpPixmap;
  Picture bmpPicture;
  GC      gc;
  
  bmpPixmap = XCreatePixmap (dpy, DefaultRootWindow(dpy),
			     width,
			     height,
			     32);
  if (!bmpPixmap){
  ...
  bmpPicture = XRenderCreatePicture (dpy, bmpPixmap,
				     XRenderFindStandardFormat(dpy, PictStandardARGB32),
				     0, NULL);
  if (!bmpPicture)
    {
      XDestroyImage (bmpImage);
      XFreePixmap (dpy, bmpPixmap);
      return (Picture)None;
    }

  gc = XCreateGC (dpy, bmpPixmap, 0, NULL);
  if (!gc)
    {
      XFreePixmap (dpy, bmpPixmap);
      XRenderFreePicture (dpy, bmpPicture);
      return (Picture)None;
    }

  XPutImage (dpy, bmpPixmap, gc, bmpImage, 0, 0, 0, 0,
	     bmpImage->width,
	     bmpImage->height);
  return bmpPicture;
}

static Picture AMap_Builder(Display *dpy, XImage *WPicture, unsigned char *WinData, double opacity){
  //building window map
  WinData = (unsigned char*)malloc(WPicture->width * WPicture->height * sizeof(unsigned char));
  int WParser = 0;
  for(int i = 0; i<WPicture->height * WPicture->width; i++){
    if((unsigned char)WPicture->data[WParser+3]>255/3)
      WinData[i]=255*opacity;
    else
      WinData[i]=WPicture->data[WParser+3];
    WParser+=4;
  }
  ...
  if(has_corners){
    int xus, yus, coord;
    for(int a = 0; a<=90; a++){
      xus = corner_radius * cos(a*M_PI/180) + WPicture->width  - corner_radius;
      yus = corner_radius * sin(a*M_PI/180) + WPicture->height - corner_radius;

      coord = WPicture->width * yus + xus;
      WinData[coord]=0;
    
      for(int i=yus+1; i<WPicture->height; i++){
	coord = WPicture->width * i + xus;
	WinData[coord]=0;
      }
    }

    for(int a = 91; a<=180; a++){
      xus = corner_radius * cos(a*M_PI/180) + corner_radius;
      yus = corner_radius * sin(a*M_PI/180) + WPicture->height - corner_radius;
    ...
    for(int a = 181; a<=270; a++){
      xus = corner_radius * cos(a*M_PI/180) + corner_radius;
      yus = corner_radius * sin(a*M_PI/180) + corner_radius;

      coord = WPicture->width * yus + xus;
      WinData[coord]=0;
    
      for(int i=yus-1; i>=0; i--){
	coord = WPicture->width * i + xus;
	WinData[coord]=0;
      }
    }

    for(int a = 271; a<=360; a++){
      xus = corner_radius * cos(a*M_PI/180) + WPicture->width  - corner_radius;
      yus = corner_radius * sin(a*M_PI/180) + corner_radius;
  ...
  //transforming window map in XImage
  XImage *WImage = XCreateImage(dpy, XRenderFindStandardFormat(dpy, PictStandardA8),
				8, ZPixmap, 0, WinData,
			        WPicture->width, WPicture->height,
				8, 0);

  //transforming XImage into a Picture
  Pixmap  WinPixmap;
  Picture MaskPicture;
  GC      gc;
  
  WinPixmap = XCreatePixmap (dpy, DefaultRootWindow(dpy),
			     WPicture->width,
			     WPicture->height,
			     8);
  if (!WinPixmap){
  ...
  MaskPicture = XRenderCreatePicture (dpy, WinPixmap,
				     XRenderFindStandardFormat(dpy, PictStandardA8),
				     0, NULL);
  if (!MaskPicture)
    {
      XDestroyImage (WImage);
      XFreePixmap (dpy, WinPixmap);
      return (Picture)None;
    }

  gc = XCreateGC (dpy, WinPixmap, 0, NULL);
  if (!gc)
    {
      XFreePixmap (dpy, WinPixmap);
      XRenderFreePicture (dpy, MaskPicture);
      return (Picture)None;
    }

  XPutImage (dpy, WinPixmap, gc, WImage, 0, 0, 0, 0,
	     WPicture->width,
	     WPicture->height);
...
static Picture AMap_Builder24(Display *dpy, int width, int height, unsigned char *WinData, double opacity){
  //building window map
  WinData = (unsigned char*)malloc(width * height * sizeof(unsigned char));
  ...
  int xus, yus, coord;
  for(int a = 0; a<=90; a++){
    xus = corner_radius * cos(a*M_PI/180) + width  - corner_radius;
    yus = corner_radius * sin(a*M_PI/180) + height - corner_radius;

    coord = width * yus + xus;
    WinData[coord]=0;
    
    for(int i=yus+1; i<height; i++){
      coord = width * i + xus;
      WinData[coord]=0;
    }
  }
...
static void
paint_all (Display *dpy, XserverRegion region, int NormalComp)
{
    win	*w;
    win	*t = NULL;
    //XEvent esafe;
	...
	case CompClientShadows:
	    ...
	    {
		XRenderComposite (dpy, PictOpSrc, blackPicture, w->shadow, rootBuffer,
				  0, 0, 0, 0,
				  w->a.x + w->shadow_dx,
				  w->a.y + w->shadow_dy,
				  w->shadow_width, w->shadow_height);
	...
	if (w->mode == WINDOW_TRANS)
	{
	    ...
	    set_ignore (dpy, NextRequest (dpy));
	    if(w->id){
	      XImage *test_alpha = XGetImage(dpy, w->id, 0, 0, wid, hei, AllPlanes, ZPixmap);
	      
	      if(test_alpha)
		{
		  unsigned char *to_free;
		  Picture tempAlpha = AMap_Builder(dpy, test_alpha, &to_free, (double)w->opacity/OPAQUE);

		  set_ignore(dpy, NextRequest(dpy));
		
		  XRenderComposite (dpy, PictOpOver, underBKG, tempAlpha, rootBuffer,
				    x, y, 0, 0,
				    x, y, wid, hei);
		
		  set_ignore(dpy, NextRequest(dpy));
		  XRenderComposite (dpy, PictOpOver, w->picture, tempAlpha, rootBuffer,
				    0, 0, 0, 0,
				    x, y, wid, hei);
		
		  XRenderFreePicture(dpy, tempAlpha);
		  free(to_free);
		  free(test_alpha->data);
		}
	    }
	    else{
	      set_ignore(dpy, NextRequest(dpy));
	      XRenderComposite (dpy, PictOpOver, underBKG, w->alphaPict, rootBuffer,
				x, y, 0, 0,
				x, y, wid, hei);
	...
	else if (w->mode == WINDOW_ARGB)
	{
	    ...
	    if(w->id && NormalComp && use_bkg_pic){
	      XImage *test_alpha = XGetImage(dpy, w->id, 0, 0, wid, hei, AllPlanes, ZPixmap);
		...
		free(test_alpha->data);
	      ...
	      Picture tempAlpha24;
	      if(!has_corners)
		tempAlpha24 = solid_picture (dpy, False,
					     (double) w->opacity / OPAQUE, 0, 0, 0);
	      else
		tempAlpha24 = AMap_Builder24(dpy, wid, hei, &to_free, (double)w->opacity/OPAQUE);
	      
	      set_ignore(dpy, NextRequest(dpy));
	      
	      if(use_bkg_pic)
		XRenderComposite (dpy, PictOpOver, underBKG, tempAlpha24, rootBuffer,
				  x, y, 0, 0,
				  x, y, wid, hei);
	      
	      XRenderComposite (dpy, PictOpOver, w->picture, tempAlpha24, rootBuffer,
				0, 0, 0, 0,
				x, y, wid, hei);
	      XRenderFreePicture(dpy, tempAlpha24);
	      free(to_free);
	    }
	}
	XFixesDestroyRegion (dpy, w->borderClip);
...
usage (const char *program)
{
    ...
    "   -P\n"
    "	   Use the image from /usr/local/etc/XCompEvolution/bkg.bmp as background for transparent windows\n"
    "   -d display\n"
...
register_cm (Display *dpy)
{
    ...
    Xutf8SetWMProperties (dpy, w, "xcompEvolution", "xcompEvolution", NULL, 0, NULL, NULL,
			  NULL);
...
main (int argc, char **argv)
{
    ...
    int		    o;
     ...
     /* Set Background */
    //creating picture from image
    dpy = XOpenDisplay(display);
    
    while ((o = getopt (argc, argv, "R:D:I:O:d:r:o:l:t:scnfFCaSP")) != -1)
    {
	switch (o) {
	case 'R':
	    has_corners = 1;
    ...
    if(use_bkg_pic){
      unsigned char *btfree;
      backOfTheWindow = BMP_READER("/usr/local/etc/XCompEvolution/bkg.bmp", dpy, &hdata,
				   &wdata, &ddata, btfree);
      //printf("%d, %d \n\n", wdata, hdata);
      underBKG = Bitmap_Transformer(dpy, backOfTheWindow, hdata, wdata, ddata, btfree);
    }
